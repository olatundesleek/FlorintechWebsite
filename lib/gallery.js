...
function sortGalleryRecordsByNewest(records = []) {
  return toSafeArray(records).sort((first, second) => {
    const firstDate = first?.createdAt || first?.created_at || 0;
    const secondDate = second?.createdAt || second?.created_at || 0;
    return new Date(secondDate) - new Date(firstDate);
  });
}

export function getPublishedGalleryItems(items = []) {
  return sortGalleryRecordsByNewest(
    toSafeArray(items).filter(
      (item) => item && typeof item === "object" && item.published !== false
    )
  );
}
...
export function getLatestGalleryItemsByCategory(items = [], categoryNames = [], limit = 4) {
  ...
  const orderedCategories = (validCategories.length ? validCategories : Object.keys(latestByCategory))
    .filter((category) => latestByCategory[category])
    .sort((firstCategory, secondCategory) => {
      const firstDate = new Date(latestByCategory[firstCategory]?.createdAt || 0).getTime();
      const secondDate = new Date(latestByCategory[secondCategory]?.createdAt || 0).getTime();
      return secondDate - firstDate;
    });

  return orderedCategories.slice(0, limit).map((category) => latestByCategory[category]);
}
...
function getGalleryApiUrl({ page, limit, category }) {
  const baseUrl =
...
export async function getGalleryPage({ page = 1, limit = 24, category = "All" } = {}) {
  const apiUrl = getGalleryApiUrl({ page, limit, category });
...
export async function getGalleryItems() {
  const galleryPage = await getGalleryPage({ page: 1, limit: 100 });
